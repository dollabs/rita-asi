...
exp_script = Path(codebase).joinpath('Code/evaluations/study-2/bin/do_experiment.sh')
...
def get_exp_dirs(top_dir):
    ...
    n_exps = len(all)
    if n_exps < npars:
        sze = 1 # Run all experiments in parallel
    else:
        sze = int(len(all) / npars)
        
    for sample in make_chunks(all, sze):
        # pprint(sample)
        acc.append(sample)
    return acc
