...
(ns plantInterface.DPLinterface-with-binding
  ...
  (:require [clojure.string :as string]
            ...
            [rmqConnection.RMQConnectionPlant :as plantRMQ])
  (:gen-class))

;;; DOLL Plant Learning Interface
(defrecord dplinterface [;; World-specific parameters
                         world-parameters ; a list of arguments
                         ;; RabbitMQ information (This is the exchange-name)
                         routing binding channel exchange plantid
...
(defn maybe-cancel-subscription
  [ctag]
  (when last-ctag
    (plantRMQ/cancel-subscription (first last-ctag) (second last-ctag)))
...
(defn await-plant-message
  ...
  (locking fresh-data-lock
    (def fresh-data-promises (merge fresh-data-promises {plant-id apromise}))))                    ; Block until the data is ready
      ...
      (cond
        ...
        (if true #_(= plantid plantifid)
            (do
              ...
              (doseq [anobs observations] ;; TTT a loop thru each value in observations
              ...
              (let [apromise (locking fresh-data-lock
                               (let [oldpromise (get fresh-data-promises plantid)]
                                ;;  (println "PROMISE:" fresh-data-promises)
...
(defn rabbitMQ-connect
  ...
  (let [connection (rmq/connect {:host host :port port})
        ...
        exchange (le/declare channel ex-name "topic" {:durable false :auto-delete true})
        queue' (:queue (lq/declare channel "" {:exclusive false :auto-delete true}))
        _ (lq/bind channel queue' ex-name {:routing-key binding-key}) ;; listening to all routing keys? (hash) can substitute for zero or more words.
        lctag (lc/subscribe channel queue' incoming-msgs {:auto-ack true})]
...
(defn bp-call
  ...
  (let [id (do (locking call-lock
        ...
        {exchange :exchange routing :routing binding :binding channel :channel} self
        finished (promise)]

    (locking call-lock
      (def running-functions (merge running-functions {id finished}))
      (println id :function-name function))

    (plantRMQ/publishJSON channel exchange routing
                          {:id id
                           :plant-id pid
                           :routing-key routing
                           :binding-key binding
                           ...
                           :args args})
    (deref finished))) ;; Ask Paul about this later
;;; Fin
