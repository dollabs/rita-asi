...
class Rmq:
    ...
    def send_message(self, msg, routing_key):
        # print('rmq message enq', threading.current_thread().name)
        self.__enque_to_rmq(self.exchange, routing_key, json.dumps(msg))

    def send_message_now(self, msg, routing_key):
        self.__basic_publish(self.exchange, routing_key, json.dumps(msg))
    ...
    def process_to_rmq(self):
        ...
        if n_waiting > 0:
            # print('process_to_rmq waiting', n_waiting, threading.current_thread().name)
