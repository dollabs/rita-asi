...
class GridWorld:
    ...
    def available_actions(self):
        actions = []
        pos = self._pos_agent
        for action in list(range(self.nActions())):
            if (action == 0):
                if (self._map[pos[0] - 1, pos[1]] == 0):
                    actions.append(action) # go up
            elif (action == 1):
                if (self._map[pos[0] + 1, pos[1]] == 0):
                    actions.append(action) # go down
            elif (action == 2):
                if (self._map[pos[0], pos[1] - 1] == 0):
                    actions.append(action) # go left
            elif (action == 3):
                if (self._map[pos[0], pos[1] + 1] == 0):
                    actions.append(action) # go right
        return actions
    ...
    def act(self, action):
        ...
        if (action == 0):
            if (self._map[self._pos_agent[0] - 1, self._pos_agent[1]] == 0):
                self._pos_agent[0] = self._pos_agent[0] - 1 # go up
        elif (action == 1):
            if (self._map[self._pos_agent[0] + 1, self._pos_agent[1]] == 0):
                self._pos_agent[0] = self._pos_agent[0] + 1 # go down
        elif (action == 2):
            if (self._map[self._pos_agent[0], self._pos_agent[1] - 1] == 0):
                self._pos_agent[1] = self._pos_agent[1] - 1 # go left
        elif (action == 3):
            if (self._map[self._pos_agent[0], self._pos_agent[1] + 1] == 0):
                self._pos_agent[1] = self._pos_agent[1] + 1 # go right
