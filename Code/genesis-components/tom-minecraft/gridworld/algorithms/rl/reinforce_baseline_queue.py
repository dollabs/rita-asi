...
DISCOUNT_RATE = 0.99
LEARNING_RATE = 1.00
POLICY_NODES = [2 + 2*HISTORY_LENGTH, 128, 64, 3]
...
class ActorCriticNetwork(nn.Module):
    ...
    def get_action(self, state, env, trace, trace_pos):
        ...
        trace_aug = []
        if len(trace) < HISTORY_LENGTH:
            trace_aug = [state] + trace[::-1]
            trace_aug += [trace[0]]*(HISTORY_LENGTH - len(trace))
        else:
            trace_aug = [state] + trace[len(trace)-1:-(HISTORY_LENGTH + 1):-1]

        net_in = [0]*POLICY_NODES[0]
        for i in range(0, HISTORY_LENGTH + 1):
            net_in[2*i] = trace_aug[i][0]
            net_in[2*i+1] = trace_aug[i][1]
        net_in = np.array(net_in)
        net_in = torch.from_numpy(net_in).float().unsqueeze(0).to(device) # numpy list to torch.tensor
        
        # Get the policy probability vector and value function
        (p, value) = self.forward(Variable(net_in)) # multinomial probability over actions
        ...
        while next_state[0] == 'wall' or (next_state[0] in trace_pos[::-1] and move_on_counter < 3):
...
def train(continue_train=False, 
    ...
    if continue_train:
        ...
        net.load_state_dict(torch.load('reinforce_baseline_queue_net.pt'))
        net.train()
        net.to(device)
    ...
    for ep in range(episodes):
        ...
        for step in range(max_steps):
            action, log_prob, value, new_state = net.get_action(state, env, trace) 
    ...
    torch.save(net.state_dict(), 'reinforce_baseline_queue_net.pt')

def generate_trajectory(checkpoint='reinforce_baseline_queue_net.pt', 
                        max_steps=MAX_STEPS):

    env = mdp.POMDP(visualize.MAP, visualize.MAP_ROOM, player.players[visualize.PLAYER_NAME]) 

    policy_net = ActorCriticNetwork(policy_nodes=POLICY_NODES, lr=LEARNING_RATE)
    if os.path.exists('reinforce_baseline_queue_net.pt'):
    ...
    trace = []
    input_trace = [state]
    input_trace_pos = [state[0]]
    log_probs = []
    values = []

    for step in range(max_steps):
        # Get action, corresponding probability and next state from network
        action, log_prob, value, new_state = policy_net.get_action(state, env, input_trace, input_trace_pos) 
        log_probs.append(log_prob)
        values.append(value)
        ...
        input_trace.append(new_state)
        input_trace_pos.append(new_state[0])
    ...
    torch.save(policy_net.state_dict(), 'reinforce_baseline_queue_net.pt')
