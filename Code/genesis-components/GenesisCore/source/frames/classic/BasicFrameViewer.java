...
public class BasicFrameViewer extends JComponent implements MouseMotionListener, MouseListener {
	...
	private void paintFrameBundle(Graphics g, FrameBundle bundle, int x, int y, int width, int height, double preciseFontHeight) {
		...
		int topFontHeight = Math.min(fontHeight, height / 3);
		...
		if (topFontHeight > 0) {
			...
			g.drawString(bundle.getTop(), x + fontHeight + lineWidth, y + topFontHeight);
			if (bundle.isNegated()) {
		...
		if (bottomFontHeight > 0) {
			...
			g.drawString(bundle.getBottom(), x + fontHeight + lineWidth, y + height - bottomFontHeight);
			/*
	...
	public static void main(String[] args) {
		...
		FrameViewer display = new FrameViewer();
		Vector v = new Vector();
		v.add("Go");
		v.add("Move");
		v.add("Thing");
