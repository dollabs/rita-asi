...
public class FrameViewer extends JScrollPane implements WiredBox {
	// ============= CONSTANTS ============
	...
	public FrameViewer(String t, int scrollable) {
		this(t);
		setScrollable(scrollable);
	}

	// Added so you can change the border color -- MAF.26.Jan.04
	public FrameViewer(String t, Color color) {
		this();
		title = t;
	...
	public void setInput(Entity t) {
		setInput(EntityToViewerTranslator.translate(t));
	}
	...
	public void setStory(Sequence s) {
		...
		for (Entity t : v) {
			bundles.add(EntityToViewerTranslator.translate(t, EntityToViewerTranslator.SHOW_NO_THREADS));
		}
	...
	public void setInputVector(Vector v) {
		...
		for (int i = 0; i < v.size(); ++i) {
			if (v.elementAt(i) instanceof FrameBundle) {
				...
				FrameBundle bundle = EntityToViewerTranslator.translate(t);
				setInput(i, bundle);
			}
		}
		revalidate();
	...
	private void setInput(int panel, FrameBundle b) {
		if (panel >= panels.size()) {
			for (int i = panels.size(); i <= panel; ++i) {
				BasicFrameViewer bfv = new BasicFrameViewer();
				panels.add(bfv);
		...
		BasicFrameViewer target = (BasicFrameViewer) (panels.elementAt(panel));
	...
	public void setAtPreferredSize() {
		BasicFrameViewer bfv;
		Dimension size;
		if (panels == null) return;
		for (int i = 0; i < panels.size(); i++) {
			bfv = (BasicFrameViewer) panels.get(i);
	...
	public static void main(String[] ignore) {
		...
		FrameViewer view = new FrameViewer();
