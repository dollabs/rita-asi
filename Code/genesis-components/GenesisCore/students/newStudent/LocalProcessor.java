...
public class LocalProcessor extends AbstractWiredBox {
	...
	public void processStoryProcessor(Object signal) {
		...
		if (signal instanceof StoryProcessor) {
			...
			// Demonstration for Suri

			Mark.say("\n>>>\n>>>  Concept patterns");
			GenesisGetters.getMentalModel1().getStoryProcessor().getConceptPatterns().stream().forEach(p -> {
				Mark.say("\n>>>\n>>>  Concept pattern:", p.getType());
				p.getElements().stream().forEachOrdered(e -> {
					Mark.say(e);
				});
			});
		}
